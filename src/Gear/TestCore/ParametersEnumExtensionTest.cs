...
namespace TestCore
{
    ...
    public class ParametersEnumExtensionTest
    {
        private const string TestGetDescription_ReturnValue_TestName =
            "Когда вызывается метод GetDescription для параметра {2}, он должен" +
            " вернуть его описание в виде строки";

        [TestCase(ParametersEnum.GearDiameter, "Gear Diameter \"D\":",
            TestName = TestGetDescription_ReturnValue_TestName)]
        [TestCase(ParametersEnum.HoleDiameter, "Hole Diameter \"d\":",
            TestName = TestGetDescription_ReturnValue_TestName)]
        [TestCase(ParametersEnum.Height, "Height \"H\":",
            TestName = TestGetDescription_ReturnValue_TestName)]
        [TestCase(ParametersEnum.ToothLength, "Tooth Length \"A\":",
            TestName = TestGetDescription_ReturnValue_TestName)]
